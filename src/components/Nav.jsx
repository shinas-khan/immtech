    ...
    <>
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "rgba(255,255,255,0.97)", backdropFilter: "blur(20px)", borderBottom: "1px solid #E8EEFF", boxShadow: scrolled ? "0 2px 20px rgba(0,57,255,0.08)" : "none", transition: "box-shadow 0.3s" }}>
        <div style={{ maxWidth: 1140, margin: "0 auto", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 5%" }}>
          ...
          {/* Desktop links - hidden on mobile via React state */}
          {!isMobile && (
            <div style={{ display: "flex", gap: 32, alignItems: "center" }}>
              {links.map(l => (
                <span key={l.path} onClick={() => navigate(l.path)}
                  style={{ color: isActive(l.path) ? "#0057FF" : "#4B5675", fontWeight: isActive(l.path) ? 700 : 500, fontSize: 15, cursor: "pointer", transition: "color 0.2s", borderBottom: isActive(l.path) ? "2px solid #0057FF" : "2px solid transparent", paddingBottom: 2 }}
          ...
          {/* Desktop auth - hidden on mobile via React state */}
          {!isMobile && (
              ...
              {user ? (
                ...
                <>
                  <button onClick={() => navigate("/auth")}
                    style={{ background: "none", border: "1.5px solid #E8EEFF", color: "#0A0F1E", borderRadius: 9, padding: "8px 20px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
                  ...
                  <button onClick={() => navigate("/onboarding")}
                    style={{ background: "linear-gradient(135deg, #0057FF, #00C2FF)", border: "none", color: "#fff", borderRadius: 9, padding: "8px 20px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                    Get Started
                  </button>
                </>
              )}
          ...
          {/* Hamburger - only shown on mobile via React state */}
          {isMobile && (
        ...
        {/* Mobile dropdown - only renders when open AND on mobile */}
        {isMobile && menuOpen && (
          <div style={{ background: "#fff", borderTop: "1px solid #E8EEFF", padding: "12px 5% 20px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {links.map(l => (
                <div key={l.path} onClick={() => navigate(l.path)}
                  style={{ padding: "14px 16px", borderRadius: 12, background: isActive(l.path) ? "#0057FF0D" : "transparent", color: isActive(l.path) ? "#0057FF" : "#0A0F1E", fontWeight: isActive(l.path) ? 700 : 500, fontSize: 16, cursor: "pointer" }}>
                  {l.label}
                </div>
              ))}
              ...
              {user ? (
                <>
                  <div onClick={() => navigate("/profile")}
                    style={{ padding: "14px 16px", borderRadius: 12, color: "#0A0F1E", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div onClick={() => navigate("/notifications")} style={{ padding: "14px 16px", borderRadius: 12, color: "#0A0F1E", fontSize: 16, cursor: "pointer" }}>
                    &#128276; Alerts
                  </div>
